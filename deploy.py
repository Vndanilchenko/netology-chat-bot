...
text_in = None
text_out = None
schedule = schedule.Read_google_sheet_schedule()
responses_df = schedule.run(command='get_responses')
commands = ['send_info', 'show_weather', 'get_schedule', 'tell_joke', 'send_email']
classifier = models.Classifier()
state_in = {'intent': None, 'command': None, 'state': 'normal', 'reply': None, 'query_params': {}}
state_out = {'intent': None, 'command': None, 'state': 'normal', 'reply': None, 'query_params': {}}
send_email = email.Send_email()
weather_descr = pd.read_csv('./data/Multilingual_Weather_Conditions.csv', sep=',', encoding='utf-8')
...
def do_command(self, command, reply):
    ...
    if command == 'get_schedule':
        ...
        response = 'без команды зашел: {}'.format(commands)
    return response
...
def make_response(self):
    ...
    if state_in['state'] == 'approval':
        print('APPROVAL')
        # # проверка классификаторов ДА/НЕТ и отсечку по трэшолду
        max_score, max_class = classifier.predict_proba(text_in, model_flag=1)
        if max_class==1 and max_score > TRESHOLD_YES_NO:
            if state_in['command']:
                print('есть ответ по команде: {}'.format(state_in['command']))
                reply = do_command(state_in['command'], state_in['reply'])
            else:
                reply = state_in['reply']
            state_out = {'intent': state_in['intent'], 'command': state_in['command'], 'state': 'normal', 'reply': reply, 'query_params': {}}
            # return reply
        else:
            set_default_state()
            reply = negative_response()
    elif state_in['state'] == 'query':
        # проверим не захотел ли пользователь сбросить режим
        max_score, max_class = classifier.predict_proba(text_in, model_flag=2)
        if max_class==1 and max_score > TRESHOLD_USER_QUIT:
            # выходим из режима
            reply = realize_user_quit()
        else:
            ...
            reply = get_query_params()

    else:
        # prediction_kb = self.recognize_kb.kb_predict_proba(self.text_in)
        max_score, max_class = classifier.predict_proba(text_in, model_flag=0)
        ...
        if max_score > TRESHOLD_KB: # Был prediction_kb
            ...
            print('прошел СNN')
            intent, command, reply = return_intent(max_class)
            state_out = {'intent': intent, 'command': command, 'state': 'normal', 'reply': reply, 'query_params': {}}
            ...
            if max_score > TRESHOLD_EXPLICIT:
                ...
                if command:
                    ...
                    reply = do_command(command, reply)
                # return reply
            elif max_score > TRESHOLD_APPROVAL:
                state_out = {'intent': intent, 'command': command, 'state': 'approval', 'reply': reply, 'query_params': {}}
                reply = 'ты хочешь {}?'.format(intent)

            else:
                reply = terminal_response()
                set_default_state()
        else:
            reply = terminal_response()
            set_default_state()
...
def reply():
    ...
    for channel in channels:
        web_client.chat_postMessage(
            channel=channel
            ,text=text_out # + " " + str(self.direct_link)# str(self.channels)
...
def run():
    ...
    rtmclient.on(event='message', callback=detect_message())
    rtmclient.start()
